...
> Mobile spec: `mobile-implementation/04-screens/13b-GroupChat.md`
...
- **DB tables**: `courses`, `group_resources`, `notes`, `profiles`, `study_group_members`, `study_groups`
...
| `@/components/ui/avatar` | **REWRITE** | shadcn → custom RN primitive in `src/components/ui/` using NativeWind + class-variance-authority. See 05-shared-components/01-ui-primitives.md. |
...
| `@/components/chat/ChatRoom` | **PORT** | Port to RN: inverted FlatList, KeyboardAvoidingView, expo-image-picker for media. |
...
- `src/components/chat/ChatRoom.tsx` ✅ exists in web → create `mobile/src/components/chat/ChatRoom.tsx`
...
- `avatar` — implement in `mobile/src/components/ui/avatar.tsx` per `05-shared-components/01-ui-primitives.md`.
...
2. **Create the screen file** — `mobile/src/screens/GroupChatScreen.tsx`. Start from the matching `04-screens/13b-GroupChat.md` layout tree.
...
11. **Wire to navigator** — register `GroupChatScreen` in the appropriate stack/tab in `mobile/src/navigation/`.
