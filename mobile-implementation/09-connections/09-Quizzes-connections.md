...
> Mobile spec: `mobile-implementation/04-screens/09-Quizzes.md`
...
- **DB tables**: `courses`, `notes`, `quiz_attempts`
...
| `@/hooks/useActivityTracking` | **COPY** | Most hooks copy verbatim (they call supabase + react state). Drop `use-mobile.tsx` (web-only). `useMobileBackNavigation` → replace with React Navigation's useFocusEffect + BackHandler. |
| `@/components/quiz/QuizHistory` | **PORT** | Port to RN: keep scoring + history hooks. |
...
| `@/components/ui/scroll-area` | **REWRITE** | shadcn → custom RN primitive in `src/components/ui/` using NativeWind + class-variance-authority. See 05-shared-components/01-ui-primitives.md. |
| `@/lib/parseAIResponse` | **COPY** | Copy verbatim. `formatters`, `ai`, `streak`, `subscriptionConfig`, `educationConfig`, `paystackConfig`, `parseAIResponse`, `resilientFetch` all framework-agnostic. |
...
- `src/components/quiz/QuizHistory.tsx` ✅ exists in web → create `mobile/src/components/quiz/QuizHistory.tsx`
...
- `src/hooks/useActivityTracking.ts` — copy to `mobile/src/hooks/useActivityTracking.ts`. If it imports `localStorage` or `window`, replace with `AsyncStorage` / `Platform`.
...
- `scroll-area` — implement in `mobile/src/components/ui/scroll-area.tsx` per `05-shared-components/01-ui-primitives.md`.
...
2. **Create the screen file** — `mobile/src/screens/QuizzesScreen.tsx`. Start from the matching `04-screens/09-Quizzes.md` layout tree.
...
11. **Wire to navigator** — register `QuizzesScreen` in the appropriate stack/tab in `mobile/src/navigation/`.
