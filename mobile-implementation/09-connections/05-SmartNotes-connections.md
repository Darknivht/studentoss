# Connections — `SmartNotes`

> Web source: `src/pages/SmartNotes.tsx`  
> Mobile spec: `mobile-implementation/04-screens/05-SmartNotes.md`
...
| `@/components/notes/SocraticTutor` | **PORT** | Port to RN: replace div→View, button→Pressable, input→TextInput, keep all hooks + supabase calls verbatim. |
| `@/components/notes/AISummaryDialog` | **PORT** | Port to RN: replace div→View, button→Pressable, input→TextInput, keep all hooks + supabase calls verbatim. |
| `@/components/notes/FileUpload` | **PORT** | Port to RN: replace div→View, button→Pressable, input→TextInput, keep all hooks + supabase calls verbatim. |
...
| `@/hooks/useOfflineSync` | **COPY** | Most hooks copy verbatim (they call supabase + react state). Drop `use-mobile.tsx` (web-only). `useMobileBackNavigation` → replace with React Navigation's useFocusEffect + BackHandler. |
...
| `@/components/subscription/UpgradePrompt` | **PORT** | Port to RN: FeatureGateDialog → RN Modal + Moti. See 05-shared-components/03. |
...
- `src/components/notes/SocraticTutor.tsx` ✅ exists in web → create `mobile/src/components/notes/SocraticTutor.tsx`
- `src/components/notes/AISummaryDialog.tsx` ✅ exists in web → create `mobile/src/components/notes/AISummaryDialog.tsx`
- `src/components/notes/FileUpload.tsx` ✅ exists in web → create `mobile/src/components/notes/FileUpload.tsx`
- `src/components/notes/NoteViewerDialog.tsx` ✅ exists in web → create `mobile/src/components/notes/NoteViewerDialog.tsx`
- `src/components/subscription/UpgradePrompt.tsx` ✅ exists in web → create `mobile/src/components/subscription/UpgradePrompt.tsx`
...
- `src/hooks/useOfflineSync.ts` — copy to `mobile/src/hooks/useOfflineSync.ts`. If it imports `localStorage` or `window`, replace with `AsyncStorage` / `Platform`.
...
2. **Create the screen file** — `mobile/src/screens/SmartNotesScreen.tsx`. Start from the matching `04-screens/05-SmartNotes.md` layout tree.
...
11. **Wire to navigator** — register `SmartNotesScreen` in the appropriate stack/tab in `mobile/src/navigation/`.
