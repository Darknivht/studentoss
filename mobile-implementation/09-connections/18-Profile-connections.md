...
> Mobile spec: `mobile-implementation/04-screens/18-Profile.md`
...
- **DB tables**: `profiles`, `user_achievements`
...
| `@/components/profile/AvatarUpload` | **PORT** | Port to RN: AvatarUpload uses expo-image-picker + Supabase Storage. |
...
- `src/components/profile/AvatarUpload.tsx` ✅ exists in web → create `mobile/src/components/profile/AvatarUpload.tsx`
...
2. **Create the screen file** — `mobile/src/screens/ProfileScreen.tsx`. Start from the matching `04-screens/18-Profile.md` layout tree.
...
11. **Wire to navigator** — register `ProfileScreen` in the appropriate stack/tab in `mobile/src/navigation/`.
