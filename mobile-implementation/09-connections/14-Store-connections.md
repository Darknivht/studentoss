...
> Web source: `src/pages/Store.tsx`  
> Mobile spec: `mobile-implementation/04-screens/14-Store.md`
...
- **DB tables**: `profiles`, `store_resources`
...
| `@/components/store/ResourceCard` | **PORT** | Port to RN: ResourceCard → Pressable + Image. Filters → bottom sheet. |
| `@/components/store/ResourceFilters` | **PORT** | Port to RN: ResourceCard → Pressable + Image. Filters → bottom sheet. |
| `@/components/store/YouTubeSection` | **PORT** | Port to RN: ResourceCard → Pressable + Image. Filters → bottom sheet. |
...
| `@/components/ui/skeleton` | **REWRITE** | shadcn → custom RN primitive in `src/components/ui/` using NativeWind + class-variance-authority. See 05-shared-components/01-ui-primitives.md. |
...
- `src/components/store/ResourceCard.tsx` ✅ exists in web → create `mobile/src/components/store/ResourceCard.tsx`
- `src/components/store/ResourceFilters.tsx` ✅ exists in web → create `mobile/src/components/store/ResourceFilters.tsx`
- `src/components/store/YouTubeSection.tsx` ✅ exists in web → create `mobile/src/components/store/YouTubeSection.tsx`
...
- `skeleton` — implement in `mobile/src/components/ui/skeleton.tsx` per `05-shared-components/01-ui-primitives.md`.
...
2. **Create the screen file** — `mobile/src/screens/StoreScreen.tsx`. Start from the matching `04-screens/14-Store.md` layout tree.
...
11. **Wire to navigator** — register `StoreScreen` in the appropriate stack/tab in `mobile/src/navigation/`.
