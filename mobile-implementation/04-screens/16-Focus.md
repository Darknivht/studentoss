...
> **Web source:** `src/pages/Focus.tsx`  
> **RN target:** `src/screens/FocusScreen.tsx`  
> **Route name:** `Focus`  
> **Nav type:** Stack  
> **Auth required:** Yes
...
Configure focus session and block distracting apps.
...
| `Card, CardContent` from `@/components/ui/card` | component | src/components/ui/Card.tsx (RN View + NativeWind) |
| `Flame, Clock, Target, Shield, Zap, ChevronRight` (lucide) | icons | swap import to `lucide-react-native` |
...
w-12 h-12 rounded-xl bg-gradient-to-br from-primary to-accent flex items-center justify-center
...
w-6 h-6 mx-auto mb-2 text-orange-500
text-2xl font-bold text-foreground
text-xs text-muted-foreground
w-6 h-6 mx-auto mb-2 text-blue-500
...
1. Create `src/screens/FocusScreen.tsx` — copy every hook call from the web page verbatim.
2. Render a stub `<View><Text>Focus</Text></View>` and verify the route works in the navigator.
