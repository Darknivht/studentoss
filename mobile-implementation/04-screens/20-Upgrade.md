...
> **Web source:** `src/pages/Upgrade.tsx`  
...
Paystack subscription upgrade — Plus & Pro tiers.
...
| `PAYSTACK_PUBLIC_KEY, PLAN_PRICING, formatNaira` from `@/lib/paystackConfig` | lib | **keep as-is** (supabase client / formatters / config) |
...
| `Crown, Check, Sparkles, ArrowLeft, Loader2, RefreshCw` (lucide) | icons | swap import to `lucide-react-native` |
...
p-6 rounded-3xl bg-gradient-to-br from-amber-500/20 via-yellow-500/10 to-orange-500/20 border border-amber-500/30 text-center
...
text-xs text-emerald-500 font-semibold
space-y-4
absolute top-0 right-0 px-3 py-1 bg-primary text-primary-foreground text-xs rounded-bl-xl font-medium
absolute top-0 left-0 px-3 py-1 bg-emerald-500 text-white text-xs rounded-br-xl font-medium
flex items-baseline gap-2 mb-1 mt-1
...
1. Create `src/screens/UpgradeScreen.tsx` — copy every hook call from the web page verbatim.
2. Render a stub `<View><Text>Upgrade</Text></View>` and verify the route works in the navigator.
