...
> **Route name:** `CoursePage`  
> **Nav type:** Stack — params { courseId }  
> **Auth required:** Yes

## 0. One-liner

Per-course view: notes, flashcards, quizzes, progress, tutor.
...
| `ArrowLeft, FileText, Brain, GraduationCap, Plus, Loader2` (lucide) | icons | swap import to `lucide-react-native` |
| `motion` (framer-motion) | animation | rewrite with `moti` + `react-native-reanimated` |
| `useParams, useNavigate, Link` from `react-router-dom` | other | @react-navigation/native (useNavigation, useRoute) |
...
w-10 h-10 animate-spin text-primary
p-6 space-y-6
space-y-4
w-4 h-4 mr-2
...
p-4 rounded-2xl bg-card border border-border text-center
w-6 h-6 mx-auto mb-2 text-primary
text-2xl font-bold text-foreground
text-xs text-muted-foreground
w-6 h-6 mx-auto mb-2 text-accent
w-6 h-6 mx-auto mb-2 text-emerald-500
flex gap-3
flex-1 gradient-secondary text-secondary-foreground
...
text-center py-12 bg-muted/30 rounded-2xl
w-12 h-12 mx-auto mb-3 text-muted-foreground
...
1. Create `src/screens/CoursePageScreen.tsx` — copy every hook call from the web page verbatim.
2. Render a stub `<View><Text>CoursePage</Text></View>` and verify the route works in the navigator.
