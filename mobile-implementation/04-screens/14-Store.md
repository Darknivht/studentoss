...
Textbook + resource store with YouTube section.
...
| `ResourceCard` from `@/components/store/ResourceCard` | component | port to `src/components/store/ResourceCard.tsx` (RN) |
| `ResourceFilters` from `@/components/store/ResourceFilters` | component | port to `src/components/store/ResourceFilters.tsx` (RN) |
| `YouTubeSection` from `@/components/store/YouTubeSection` | component | port to `src/components/store/YouTubeSection.tsx` (RN) |
| `Skeleton` from `@/components/ui/skeleton` | component | moti-skeleton |
| `BookOpen, Video` (lucide) | icons | swap import to `lucide-react-native` |
...
```text
p-4 pb-24 max-w-4xl mx-auto space-y-4
text-2xl font-bold
...
1. Create `src/screens/StoreScreen.tsx` — copy every hook call from the web page verbatim.
2. Render a stub `<View><Text>Store</Text></View>` and verify the route works in the navigator.
