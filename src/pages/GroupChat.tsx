...
import { ArrowLeft, Users, Settings, Copy, Check, Share2, FileText, BookOpen, Plus } from 'lucide-react';
    ...
    try {
      ...
      if (membersData) {
        ...
        setMembers(membersData.map(m => ({
          ...m,
          profile: profileMap.get(m.user_id) || { display_name: null, full_name: null, avatar_url: null }
        })));
      }
      ...
      if (resourcesData) {
        ...
        const enrichedResources = await Promise.all(resourcesData.map(async (r) => {
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">
            {group.member_count} members • {group.topic || 'General'}
          </p>
        </div>
          ...
          <div className="space-y-3">
            {resources.length === 0 ? (
              <Card className="p-8 text-center">
                ...
                <p className="text-muted-foreground">No shared resources yet</p>
                <p className="text-sm text-muted-foreground">Share notes or courses with the group</p>
              </Card>
            ) : (
              resources.map((resource) => (
                <Card key={resource.id} className="p-4 flex items-center gap-3">
          ...
          <div className="space-y-3">
            {members.map((member) => (
              <Card key={member.user_id} className="p-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="font-medium">{member.profile.display_name || member.profile.full_name || 'User'}</p>
