...
const FocusSession = () => {
    ...
    const updateElapsed = () => {
      ...
      if (elapsed >= currentSession.targetDuration * 60) {
        ...
        toast({
          ...
          description: `You focused for ${currentSession.targetDuration} minutes!`,
        });
      }
    };
  ...
  const handleStartSession = async () => {
    if (blockedApps.length === 0 && platform === 'android') {
      toast({
        title: 'Select apps to block',
        description: 'Choose which apps to block during your focus session.',
        variant: 'destructive',
      });
    ...
    if (success) {
      toast({
        ...
        description: `${duration} minute focus session is now active.`,
      });
    } else {
      toast({
    ...
    <div className="p-6 space-y-6 pb-24">
      ...
      {currentSession ? (
            ...
            <CardContent className="space-y-4">
              ...
              {currentSession.blockedApps.length > 0 && (
                <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>{currentSession.blockedApps.length} apps blocked</span>
                </div>
              )}
              ...
              <div className="flex gap-3">
                {currentSession.status === 'paused' ? (
                  <Button onClick={resumeFocusSession} className="flex-1">
                    <Play className="w-4 h-4 mr-2" />
                    Resume
                  </Button>
                ) : (
                  <Button onClick={pauseFocusSession} variant="outline" className="flex-1">
        ...
        <>
          ...
          {/* Permissions Setup */}
          {platform !== 'web' && !permissionsGranted && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
                    ...
                    <CardContent className="flex items-center justify-between p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Grant permissions for app blocking
                          </p>
                        </div>
                      </div>
                      <ChevronRight className="w-5 h-5 text-muted-foreground" />
