...
import { ArrowLeft, Loader2, BookOpen, Plus, Trash2, Copy, Check, RotateCcw, ChevronDown, ChevronUp, Download } from 'lucide-react';
...
const BibliographyBuilder = ({ onBack }: BibliographyBuilderProps) => {
  ...
  const addSource = () => {
    setSources(prev => [...prev.map(s => ({ ...s, expanded: false })), createSource()]);
  };
  ...
  const toggleExpand = (id: string) => {
    setSources(sources.map(s => s.id === id ? { ...s, expanded: !s.expanded } : s));
  };
              ...
              <div key={source.id} className="rounded-xl border border-border bg-card overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    {sources.length > 1 && (
                      <Button size="icon" variant="ghost" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); removeSource(source.id); }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <div className="rounded-2xl bg-card border border-border overflow-hidden">
            <div className="p-3 bg-muted border-b border-border flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <Button size="sm" variant="ghost" onClick={() => downloadAsHTML(bibliography, `Bibliography (${style.toUpperCase()})`, `bibliography-${style}.html`)} className="h-7">
